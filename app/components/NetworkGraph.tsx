...
/** Size multiplier when a profile picture is available */
const PFP_SIZE_MULTIPLIER = 1.2
...
export function NetworkGraph({
  ...
  useEffect(() => {
    nodes.forEach((node) => {
      if (node.avatar && !imageCacheRef.current.has(node.id)) {
        const img = new Image()
        img.crossOrigin = 'anonymous' // Strava images need CORS
        img.src = node.avatar
        img.onload = () => {
  ...
  const paintNode = useCallback(
    (
      ...
      // - Current user gets 1.3× base
      // - Nodes with profile pictures get an additional 1.2× for visibility
      let radius = baseRadius
      if (isCurrentUser) {
        radius *= CURRENT_USER_SIZE_MULTIPLIER
      }
      if (hasImage) {
        radius *= PFP_SIZE_MULTIPLIER
      }

      const color = ROUTE_COLORS[node.primaryRoute] ?? ROUTE_COLORS.other
      const borderWidth =
        isCurrentUser || isSelected || isHighlighted ? 3 : 2
      ...
      if (hasImage) {
        ...
        ctx.beginPath()
        ctx.arc(x, y, radius, 0, 2 * Math.PI)
        ctx.strokeStyle = isDimmed ? `${color}66` : color
      ...
      if (isCurrentUser) {
        ...
        ctx.quadraticCurveTo(tagRectX + tagRectW, tagRectY, tagRectX + tagRectW, tagRectY + tagCorner)
        ctx.lineTo(tagRectX + tagRectW, tagRectY + tagRectH - tagCorner)
        ctx.quadraticCurveTo(tagRectX + tagRectW, tagRectY + tagRectH, tagRectX + tagRectW - tagCorner, tagRectY + tagRectH)
        ctx.lineTo(tagRectX + tagCorner, tagRectY + tagRectH)
        ctx.quadraticCurveTo(tagRectX, tagRectY + tagRectH, tagRectX, tagRectY + tagRectH - tagCorner)
        ctx.lineTo(tagRectX, tagRectY + tagCorner)
        ctx.quadraticCurveTo(tagRectX, tagRectY, tagRectX + tagCorner, tagRectY)
        ctx.closePath()
        ctx.fill()
      ...
      // Determine font size: larger for highlighted/current user, smaller for rest
      const isEmphasized = isHighlighted || isCurrentUser
      const fontSize = isEmphasized ? 9 : 6
      ...
      // Dark bg works for both themes since graph is canvas-rendered.
      ...
      // Text color: dimmed for non-highlighted, bright for highlighted/current-user
      ctx.fillStyle = isDimmed
