...
interface GroupRideOffsetChartProps {
  ...
  trimStartSec?: number; // seconds to trim from start
  trimEndSec?: number; // seconds to trim from end
}
...
function buildOffsetData(riders: GroupRideDetailRider[]): {
  data: OffsetDataPoint[];
  riderIds: string[];
  riderNameMap: Map<string, string>;
  riderColorMap: Map<string, string>;
  /** Offset to subtract from currentTime (absolute) to align with data[].time */
  timeOffset: number;
} {
  // Only riders with valid streams
  const validRiders = riders.filter(
    (r) =>
      r.streams && r.streams.latlng.length > 0 && r.streams.time.length > 0,
  );
  ...
  // Compute absolute time offsets for each rider based on their start_date.
  // Strava's time arrays are 0-indexed relative to each rider's own start,
  // so we need to account for different absolute start times.
  const riderOffsets = new Map<string, number>();
  let globalEarliestEpoch = Infinity;
  for (const r of validRiders) {
    const epoch = new Date(r.ride.start_date).getTime() / 1000;
    if (epoch < globalEarliestEpoch) globalEarliestEpoch = epoch;
  }
  for (const r of validRiders) {
    const epoch = new Date(r.ride.start_date).getTime() / 1000;
    riderOffsets.set(r.userId, epoch - globalEarliestEpoch);
  }

  // Determine global absolute time range — use the UNION of all riders'
  // time spans so the chart starts when the first rider starts and ends
  // when the last rider finishes (matching the map's time range)
  const earliest = Math.min(
    ...validRiders.map((r) => {
      const offset = riderOffsets.get(r.userId) ?? 0;
      return r.streams!.time[0] + offset;
    }),
  );
  const latest = Math.max(
    ...validRiders.map((r) => {
      const offset = riderOffsets.get(r.userId) ?? 0;
      return r.streams!.time[r.streams!.time.length - 1] + offset;
    }),
  );

  if (latest <= earliest) {
    return { data: [], riderIds, riderNameMap, riderColorMap, timeOffset: 0 };
  }
  ...
  for (let t = earliest; t <= latest; t += SAMPLE_INTERVAL_S) {
    // Collect positions for all riders at absolute time t,
    // converting to each rider's relative time for interpolation
    const positions: { userId: string; lat: number; lng: number }[] = [];
    for (const rider of validRiders) {
      const riderOffset = riderOffsets.get(rider.userId) ?? 0;
      const relativeTime = t - riderOffset;
    ...
    // Need at least 1 rider to plot (allow partial data at start/end)
    ...
    if (futureT > t) {
      ...
      for (const rider of validRiders) {
        const riderOffset = riderOffsets.get(rider.userId) ?? 0;
        const futureRelativeTime = futureT - riderOffset;
        const pos = interpolatePosition(
          rider.streams!.latlng,
          rider.streams!.time,
          futureRelativeTime,
      ...
      if (futurePositions.length > 0) {
        const futureCentroidLat =
          futurePositions.reduce((s, p) => s + p[0], 0) /
          futurePositions.length;
        const futureCentroidLng =
          futurePositions.reduce((s, p) => s + p[1], 0) /
          futurePositions.length;
  ...
  return { data, riderIds, riderNameMap, riderColorMap, timeOffset: earliest };
}
...
function OffsetTooltip({
  ...
  // Format seconds into h:mm:ss or m:ss (same as playback time)
  const seconds = typeof label === "number" ? label : 0;
  ...
  return (
    ...
    >
      <p style={{ fontWeight: 600, marginBottom: 4 }}>{formattedTime}</p>
      {payload.map((entry) => {
        ...
        return (
          ...
          >
            {riderNameMap.get(String(entry.dataKey)) ?? entry.dataKey}: {sign}
            {val.toFixed(1)}s
          </p>
        );
      })}
...
export function GroupRideOffsetChart({
  ...
  const {
    ...
    riderColorMap,
    timeOffset,
  } = useMemo(() => buildOffsetData(riders), [riders]);

  // Filter data by trim range
  const data = useMemo(() => {
    if ((trimStartSec === 0 && trimEndSec === 0) || rawData.length === 0)
      return rawData;
    const maxTime = rawData[rawData.length - 1].time;
    return rawData.filter((point) => {
      return point.time >= trimStartSec && point.time <= maxTime - trimEndSec;
    });
  }, [rawData, trimStartSec, trimEndSec]);
  ...
  // Measure the container height for ResponsiveContainer
  // (Recharts needs explicit pixel height, not CSS flex/percentage)
  ...
  return (
    <div className="group-ride-offset-chart">
      ...
      <div
        className="group-ride-offset-chart__container"
