...
function getWeekKey(dateStr: string): { key: string; weekStart: Date } {
  ...
  const weekNum = Math.ceil(
    ((monday.getTime() - jan4Monday.getTime()) / (1000 * 60 * 60 * 24) + 1) / 7,
  );
...
export function GrowthChart({
  ...
  const chartData = useMemo(() => {
    ...
    // Route filter
    const activeRoutes =
      routeCategories && routeCategories.length > 0
    ...
    for (const stat of growthData) {
      if (!stat.user_id || !riderIdSet.has(stat.user_id)) continue;
      if (stat.route_category === null) continue;
      if (!includeOther && stat.route_category === "other") continue;
      if (
        activeRoutes &&
        !activeRoutes.has(stat.route_category as RouteCategory)
      )
        continue;
      ...
      monthMap.set(stat.month, existing + (stat.ride_count ?? 0));
    }
  ...
  return (
    <div className="leaderboard__growth-chart">
      <div className="leaderboard__growth-header">
        <h2 className="leaderboard__growth-title">Commute Growth</h2>
      </div>
      ...
      {riderIds.length > 0 && (
        <div className="growth-chart__legend">
          {riderIds.map((riderId) => {
            const color =
              riderColorMap.get(riderId) ?? "var(--color-text-muted)";
