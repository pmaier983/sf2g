...
const COLUMNS: ColumnDef[] = [
  ...
  { key: 'displayName', label: 'Rider', sortable: true, tooltip: 'Rider name — click to sort by name or view their profile' },
  { key: 'maxRidesInWindow', label: 'Max Rides', sortable: true, tooltip: 'Maximum number of SF2G rides completed within the rolling window — click to view rides' },
...
export function AllTimeTable({
  ...
  const filteredData = useMemo(() => {
    ...
    return result.filter(entry => entry.displayName.toLowerCase().includes(q))
            ...
            <tr>
              {COLUMNS.map(col => (
                ...
                >
                  {col.label}
                  {col.sortable && (
                    <span className={`sort-indicator${sortKey === col.key ? '' : ' sort-indicator--placeholder'}`}>
                      {sortDir === 'asc' ? '▲' : '▼'}
                    </span>
                  )}
                </th>
          ...
          <tbody>
            {isLoading ? (
              ...
              sortedData.map((entry, idx) => (
                <AllTimeRow key={entry.userId} entry={entry} rank={idx + 1} onViewRides={onViewRides} />
              ))
            )}
          </tbody>
        </table>
...
// Row sub-component
// ---------------------------------------------------------------------------
function AllTimeRow({ entry, rank, onViewRides }: { entry: AllTimeEntry; rank: number; onViewRides?: (userId: string, dateFrom: string, dateTo: string) => void }) {
  return (
    <tr>
      {/* Rank */}
