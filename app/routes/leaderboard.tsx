...
function LeaderboardPage() {
  ...
  const {
    ...
    isLoading,
    error,
  } = useQuery(leaderboardOptions);
  const { data: pprDawnData } = useQuery(
    pprDawnRiderIdsQueryOptions({
      dateFrom: dateFrom || undefined,
      dateTo: dateTo || undefined,
    }),
  ...
  const filteredData = useMemo(() => {
    ...
    if (!hasCompoundFilters) {
      if (routes.length > 0) {
        data = data.filter((entry) =>
          routes.some(
            (r) => (entry[`${r}_count` as keyof typeof entry] as number) > 0,
          ),
        );
      }
    ...
    data = data.filter((entry) => {
      const total =
        includeOther || routes.includes("other" as RouteCategory)
  ...
  const selectedRiderArray = useMemo((): string[] => {
    if (!leaderboardData) return [];
    return leaderboardData
      .filter((e) => e.user_id != null && selectedRiderIds.has(e.user_id))
      .map((e) => e.user_id)
      .filter((id): id is string => id != null);
  ...
  const riderColorMap = useMemo(() => {
    ...
    selectedRiderArray.forEach((id, i) => {
      if (id) map.set(id, RIDER_COLORS[i % RIDER_COLORS.length]);
    });
    return map;
  }, [selectedRiderArray]);
    ...
    if (leaderboardData) {
      for (const entry of leaderboardData) {
        if (entry.user_id)
          map.set(
            entry.user_id,
            entry.display_name ?? entry.username ?? "Rider",
          );
      }
    }
    return map;
  ...
  const allTimeRiderColorMap = useMemo(() => {
    ...
    ids.forEach((e, i) =>
      map.set(e.userId, RIDER_COLORS[i % RIDER_COLORS.length]),
    );
    return map;
  }, [allTimeQuery.data]);
  ...
  const handleDeselectAllRiders = useCallback(
  ...
  return (
    <section
      className={`leaderboard${chartOpen ? " leaderboard--chart-open" : ""}`}
    >
      ...
      <div className="leaderboard__toolbar">
        ...
        >
          <span className="filter-chips__clear-icon" aria-hidden="true">
            ✕
          </span>
          Clear Filters
        </button>
            ...
            {view === "alltime" ? (
              ...
              >
                ...
                <p className="empty-state__description">
                  Ride data will appear here once riders have synced their
                  Strava history.
                </p>
              </div>
            )}
