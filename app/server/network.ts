...
export const fetchRiderNetwork = createServerFn({ method: 'GET' }).handler(
  async (): Promise<NetworkData> => {
    ...
    const { data: coRides, error: coRideError } = await supabase
      ...
      .select(
        'rider1_id, rider2_id, route_category, polyline1, polyline2',
      )

    if (coRideError) {
      throw new Error(
        `Failed to fetch co-ride data: ${coRideError.message}`,
      )
    }
    ...
    if (riderError) {
      throw new Error(
        `Failed to fetch rider data: ${riderError.message}`,
      )
    }
    ...
    for (const rider of riders ?? []) {
      riderMap.set(rider.user_id, rider)
    }

    // 4. Layer 3: Run polyline overlap analysis + aggregate into edges
    const edgeMap = new Map<
      string,
      { source: string; target: string; weight: number; routes: Partial<Record<RouteCategory, number>> }
    >()
    ...
    // 7. Build nodes array (include ALL riders, even those with 0 connections)
    const nodes: NetworkNode[] = (riders ?? []).map((rider) => ({
      id: rider.user_id,
      name: rider.display_name ?? 'Anonymous',
      avatar: rider.avatar_url,
      totalRides: rider.sf2g_total ?? 0,
      primaryRoute: getPrimaryRoute(rider),
