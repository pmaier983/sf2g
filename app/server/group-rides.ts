...
import type { RouteCategory, Ride, RideStreamInsert } from '../lib/database.types'
...
export interface StreamFetchError {
  userId: string
  displayName: string
  type: 'RATE_LIMITED_15MIN' | 'RATE_LIMITED_DAILY' | 'REAUTH_REQUIRED' | 'PRIVATE_ACTIVITY' | 'FETCH_ERROR'
  message: string
}
 ...
 */
function parseRateLimitType(headers: Headers): 'RATE_LIMITED_15MIN' | 'RATE_LIMITED_DAILY' {
...
function clusterIntoGroups(
  pairs: Array<{ rider1Id: string; rider2Id: string; ride1Id: string; ride2Id: string; date: string; route: RouteCategory }>,
): Array<{ riderIds: Set<string>; rideIds: Set<string>; date: string; route: RouteCategory }> {
  ...
  const groups: Array<{ riderIds: Set<string>; rideIds: Set<string>; date: string; route: RouteCategory }> = []
  ...
  for (const [, datePairs] of byDateRoute) {
    ...
    for (const pair of datePairs) {
      ...
      if (!riderRides.has(pair.rider1Id)) riderRides.set(pair.rider1Id, new Set())
      if (!riderRides.has(pair.rider2Id)) riderRides.set(pair.rider2Id, new Set())
...
export const fetchGroupRides = createServerFn({ method: 'GET' })
  .inputValidator((input: { page?: number; sortBy?: string; sortDir?: 'asc' | 'desc'; dateFrom?: string; dateTo?: string }) => input)
  .handler(async ({ data }): Promise<GroupRidesResponse> => {
    ...
    const { data: coRides, error: coRideError } = await supabase
      ...
      .select('ride1_id, rider1_id, ride2_id, rider2_id, route_category, ride_date, polyline1, polyline2')
    ...
    const validPairs: Array<{ rider1Id: string; rider2Id: string; ride1Id: string; ride2Id: string; date: string; route: RouteCategory }> = []
    ...
    const { data: ridesData, error: ridesError } = await supabase
      .from('rides')
      .select('id, user_id, ride_date, route_category, average_speed_mps, max_speed_mps, distance_meters, elevation_gain_meters, average_watts, max_watts, average_heartrate, max_heartrate, moving_time_seconds')
    ...
    for (const group of groups) {
      ...
      const speeds = groupRides.map((r) => r.average_speed_mps).filter((v): v is number => v != null)
      const maxSpeeds = groupRides.map((r) => r.max_speed_mps).filter((v): v is number => v != null)
      const watts = groupRides.map((r) => r.average_watts).filter((v): v is number => v != null)
      const maxWatts = groupRides.map((r) => r.max_watts).filter((v): v is number => v != null)
      const heartrates = groupRides.map((r) => r.average_heartrate).filter((v): v is number => v != null)
      const distances = groupRides.map((r) => r.distance_meters).filter((v): v is number => v != null)
      const elevations = groupRides.map((r) => r.elevation_gain_meters).filter((v): v is number => v != null)

      const avg = (arr: number[]) => arr.length > 0 ? arr.reduce((s, v) => s + v, 0) / arr.length : 0
      const max = (arr: number[]) => arr.length > 0 ? Math.max(...arr) : 0
      const sum = (arr: number[]) => arr.reduce((s, v) => s + v, 0)

      const riders: GroupRideRider[] = Array.from(group.riderIds).map((riderId) => {
      ...
      const id = await generateGroupRideId(group.date, group.route, Array.from(group.riderIds))
    ...
    groupRideSummaries.sort((a, b) => {
      ...
      switch (sortBy) {
        case 'riderCount':
          va = a.riderCount; vb = b.riderCount; break
        case 'avgSpeedMps':
          va = a.avgSpeedMps; vb = b.avgSpeedMps; break
        case 'avgWatts':
          va = a.avgWatts ?? -1; vb = b.avgWatts ?? -1; break
        case 'avgHeartrate':
          va = a.avgHeartrate ?? -1; vb = b.avgHeartrate ?? -1; break
        case 'maxWatts':
          va = a.maxWatts ?? -1; vb = b.maxWatts ?? -1; break
        case 'maxSpeedMps':
          va = a.maxSpeedMps; vb = b.maxSpeedMps; break
        case 'totalDistanceMeters':
          va = a.totalDistanceMeters; vb = b.totalDistanceMeters; break
        case 'totalElevationMeters':
          va = a.totalElevationMeters; vb = b.totalElevationMeters; break
        case 'date':
        default:
    ...
    // 8. Paginate
    const totalCount = groupRideSummaries.length
    const start = (page - 1) * PAGE_SIZE
    const paged = groupRideSummaries.slice(start, start + PAGE_SIZE)

    return { groupRides: paged, totalCount, page, pageSize: PAGE_SIZE }
  })
...
async function fetchAndCacheStravaStreams(
  ...
  displayName: string,
): Promise<{ streams: { latlng: [number, number][]; time: number[]; watts?: number[]; heartrate?: number[] } | null; error?: StreamFetchError }> {
  ...
  if (response.status === 429) {
    ...
    return {
      ...
      error: {
        ...
        displayName,
        type: limitType,
        message: limitType === 'RATE_LIMITED_DAILY'
  ...
  const streamData = (await response.json()) as Record<string, { data: unknown[] }>
  ...
  if (upsertError) {
    console.error(`[group-rides] Failed to cache streams for ride ${rideId}:`, upsertError.message)
  ...
  return {
    streams: { latlng, time, watts: watts ?? undefined, heartrate: heartrate ?? undefined },
    error: approaching
      ? {
          userId,
          displayName,
          type: 'RATE_LIMITED_15MIN',
          message: 'Approaching Strava rate limit. Some streams may not be fetched.',
        }
      : undefined,
  }
}
...
export const fetchGroupRideDetail = createServerFn({ method: 'GET' })
  .inputValidator((input: { id: string; date: string; route: RouteCategory; riderIds: string[] }) => input)
  .handler(async ({ data }): Promise<GroupRideDetail> => {
    const supabase = createAnonClient()

    // 1. Verify the deterministic ID matches
    const expectedId = await generateGroupRideId(data.date, data.route, data.riderIds)
    ...
    const cachedStreamMap = new Map<string, { latlng: [number, number][]; time: number[]; watts?: number[]; heartrate?: number[] }>()
    for (const cached of (cachedStreams ?? []) as Array<{ ride_id: string; latlng_stream: [number, number][]; time_stream: number[]; watts_stream: number[] | null; heartrate_stream: number[] | null }>) {
    ...
    const rideStreamMap = new Map<string, { latlng: [number, number][]; time: number[]; watts?: number[]; heartrate?: number[] } | null>()
    ...
    for (const ride of rides) {
      ...
      if (result.error) {
        ...
        if (result.error.type === 'RATE_LIMITED_15MIN' || result.error.type === 'RATE_LIMITED_DAILY') {
          hitRateLimit = true
        }
      }
    }
