...
export const handleStravaCallback = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    try {
      ...
      console.log(
        `[auth] Triggering initial sync for user ${user.id} (${user.display_name ?? user.username ?? "new user"})`,
      );
      syncResult = await performSync(user.id, { skipWindEnrichment: true });
      console.log(
        `[auth] Initial sync complete: ${syncResult.newRides} new rides, ${syncResult.totalProcessed} total`,
      );
    } catch (err) {
 ...
 */
export async function cleanupDeauthorizedUser(
  userId: string,
): Promise<{ deletedRides: number }> {
  const supabase = createServiceClient();
  ...
  if (deleteError) {
    console.error(
      `[auth] Failed to delete rides for user ${userId}:`,
      deleteError.message,
    );
  ...
  try {
    ...
    console.error(
      "[auth] Failed to refresh leaderboard after ride deletion:",
      err,
    );
  }
  ...
  async () => {
    ...
    try {
      ...
      try {
        ...
        console.error(
          "Failed to revoke Strava token, continuing with local cleanup",
          err,
        );
      }
    } catch (err) {
      ...
      console.error(
        "Failed to get valid token for revocation, continuing with local cleanup",
        err,
      );
    }
