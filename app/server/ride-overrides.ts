...
export const upsertRideOverride = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    try {
      await supabase
        .from('ride_override_audit_log')
        .insert({
          ride_id: data.rideId,
          user_id: session.userId,
          action: 'upsert' as const,
          override_name: data.overrideName ?? null,
          override_route_category: data.isNotSf2g ? null : (data.overrideRouteCategory ?? null),
          is_hidden: data.isHidden ?? false,
          is_not_sf2g: data.isNotSf2g ?? false,
        })
    } catch (err) {
      // Non-critical — log but don't fail the operation
      console.warn('[ride-overrides] Failed to write audit log:', err)
    }
