...
export const fetchAllTimeLeaderboard = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data }): Promise<AllTimeEntry[]> => {
    ...
    const durationDays = Math.max(1, Math.min(Math.round(data.durationDays), 3650))
    ...
    if (data.routes && data.routes.length > 0) {
      const validCats = data.routes.filter(c => VALID_ROUTE_CATEGORIES.has(c))
    ...
    const { data: rides, error } = await query.order('ride_date', { ascending: true })
    if (error) {
    ...
    results.sort((a, b) => b.maxRides - a.maxRides || b.totalRides - a.totalRides)
