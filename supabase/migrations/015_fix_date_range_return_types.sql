-- ============================================================
-- Migration 015: Fix get_leaderboard_by_date_range return types
-- ============================================================
-- Multiple type mismatches between the function body's SELECT
-- and the RETURNS TABLE declaration:
--
-- 1. SUM(real) returns real, not DOUBLE PRECISION → add ::DOUBLE PRECISION casts
-- 2. MAX/MIN on date column returns date, not text → add ::TEXT casts
-- 3. ride_date is DATE type, not TEXT → remove unnecessary ::date casts
-- 4. Changed from SECURITY DEFINER to SECURITY INVOKER
--
-- Fix: add explicit casts on all aggregation results.
...
BEGIN
  ...
  SELECT
    ...
    MAX(r.ride_date)::TEXT AS last_ride_date,
    MIN(r.ride_date)::TEXT AS first_ride_date,
    COALESCE(
