-- ============================================================
-- Migration 031: Add avg_watts + avg_heartrate to leaderboard
-- ============================================================
-- Adds two new aggregate columns to leaderboard_view and
-- get_leaderboard_by_date_range:
--   avg_watts      — average watts across SF2G rides (power meter rides only)
--   avg_heartrate  — average heart rate across SF2G rides (HR monitor rides only)

-- 1. Recreate the materialized view with new columns
...
SELECT
  ...
  -- NEW: average watts (SF2G rides with power meter data only)
  ...
  -- NEW: average heart rate (SF2G rides with HR monitor data only)
...
-- 2. Recreate the date-range RPC function with new columns
